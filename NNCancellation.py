...
#x 是发射端发射的信号
#y 是接受端接受到的自干扰信号 其中包括线性部分和非线性部分
...
yOrig = y_train
y_train = y_train - yCanc #把y_train 变成只有非线性部分的自干扰信号
yVar = np.var(y_train)
y_train = y_train/np.sqrt(yVar) #为了训练更稳定,之后需要还原从而与y_test比较
...
yCancNonLin = np.squeeze(pred[0] + 1j*pred[1], axis=1)  #这是预测的自干扰信号的非线性部分
